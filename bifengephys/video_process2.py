...
def calib_traj(traj_df, bp='head', XYMAX=400, THRL=0.95, THRD=5):
    scorer = traj_df.columns[0][0]
    ...
    # duration = length / (60.0*fps)
    # start_frame = int(start_time * fps)  # start frame for evaluation
    # # number of the last frame for evaluation, duration is time in minutes
    # end_frame = int(start_frame + duration * 60 * fps)
    ...
    # the video frame is 330 by 330 pixels in size in this instance
    XMIN = 0
    XMAX = XYMAX
    YMIN = 0
    YMAX = XYMAX
    ...
    plt.plot(bd_x, bd_y, label='unprocessed trajectory')
    plt.plot(bd_x_new, bd_y_new, alpha=0.4, label='processed trajectory')
...
def calculate_speed_acc(traj_x, traj_y, fps, move_cutoff=5, period=5):
    ...
    speed = inst_distance * fps  # unit: pixels per second
...
def get_rois_dists(traj_x, traj_y):
    roi_at_each_frame = []
    rois = []
    dists = []
...
def get_open_closed_events(transitions, prev, frame_trans, fps=50):
...
def get_lingerings(transitions, prev, frame_trans, fps=50):
    ...
    return lingering_entrytime, lingering_exittime

def get_nose_dips(dists, fps=50):
    dip_starttime = []
    dip_stoptime = []
...
def find_transition_ezm(traj_x, traj_y, fps=50):
    # ROI Analysis
    rois, roi_at_each_frame, data_time_inrois, dists = get_rois_dists(traj_x, traj_y)
    ...
    transitions, prev, frame_trans = get_transitions(rois)

    open_closed_entrytime, open_closed_exittime, closed_open_entrytime, closed_open_exittime = get_open_closed_events(transitions, prev, frame_trans)
    ...
    lingering_entrytime, lingering_exittime = get_lingerings(transitions, prev, frame_trans)
    print("Number of lingerings in transition region detected: %d" % len(lingering_entrytime))

    dip_starttime, dip_stoptime = get_nose_dips(dists)
...
def behavior_process(session, behavior='ezm', start_time=0, duration=600, bp='head', fps=50):
    fps = fps
    traj_df, scorer = load_traj(session)
    traj_x, traj_y = calib_traj(traj_df, bp=bp)

    accumulated_distance, spd, acc, ismoving = calculate_speed_acc(traj_x, traj_y, fps, move_cutoff=5)
    ...
    if behavior == 'ezm':
        rois_stats, transitions = find_transition_ezm(traj_x, traj_y)
