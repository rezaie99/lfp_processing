...
def calib_traj(traj_df, start_time=0, duration=15, fps=50, bp='head', XYMAX=400, THRL=0.95, THRD=5):
    scorer = traj_df.columns[0][0]
...
def find_transition_ezm(traj_x, traj_y, start_time, duration, fps=50):
    # ROI Analysis (EZM)
    length = len(traj_x)
    # start_time unit: seconds
    # index of the first frame for evaluation
    ...
    lingering_entrytime, lingering_exittime = get_lingerings(transitions, prev, frame_trans, start_frame, end_frame)
...
def behavior_process(session, start_time=0, duration=10, behavior='ezm', bp='head', fps=50):
    fps = fps
    ...
    if behavior == 'ezm':
        rois_stats, transitions = find_transition_ezm(traj_x, traj_y, start_time, duration, fps)
